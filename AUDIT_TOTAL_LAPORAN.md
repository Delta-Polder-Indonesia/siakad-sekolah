...
| BUG-06 | Backend tsc gagal (P2) | 🔶 **SEBAGIAN (CI menemukan & sudah difix)** | CI (`npx prisma generate` → `tsc`) memakai client Prisma asli. Muncul 2 jenis error TS nyata & sudah diperbaiki: (1) `beforeEach(() => vi.clearAllMocks())` mengembalikan `VitestUtils` di 8 file test → diganti blok kurung kurawal; (2) kolom JSON `content` di `assignment.service.ts` tak terima `null` → helper `jsonInput()` memakai `Prisma.DbNull`/`InputJsonValue`. Service non-test lain lolos tsc. Sandbox tetap tak bisa `prisma generate` (network); konfirmasi final via re-run CI. |
