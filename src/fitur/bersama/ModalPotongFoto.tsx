...
export default function ModalPotongFoto({
  ...
  useEffect(() => {
    ...
    setGeserX((current) =>
      Math.max(-perhitungan.maxGeserX, Math.min(perhitungan.maxGeserX, current))
    );
    setGeserY((current) =>
      Math.max(-perhitungan.maxGeserY, Math.min(perhitungan.maxGeserY, current))
    );
  ...
  const warnaTombol =
    warnaAksen === 'biru' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-emerald-600 hover:bg-emerald-700';
      ...
      <div className="w-full max-w-xl rounded-2xl bg-white p-5 shadow-2xl md:p-6">
        ...
        <div className="grid gap-4">
          <div
            ...
            style={{ width: UKURAN_PREVIEW, height: UKURAN_PREVIEW }}
          >
            {perhitungan ? (
              <img
                src={sumberGambar}
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            ...
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm text-white ${warnaTombol}`}
            >
              Simpan Foto
            </button>
          </div>
