...
export default function Ebook6Page() {
  const goBack = useBackNavigation();

  const handleDownload = () => {
    const link = document.createElement('a');
    link.href = `${import.meta.env.BASE_URL}ebook/ebook_6.pdf`;
    link.download = '';
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-100 font-sans text-slate-900">
      <header className="sticky top-0 z-50 flex h-14 items-center gap-3 bg-blue-800 px-4 shadow-md lg:px-8">
        ...
        <div>
          <span className="block text-base leading-none font-bold text-white">Ebook 6</span>
          <span className="text-[11px] text-white/70">E-Book Perpustakaan</span>
        </div>
      </header>
      <div className="mx-auto max-w-4xl px-4 py-12">
        <h1 className="text-3xl font-bold">Ebook 6</h1>
        <p className="mt-4 text-slate-600">Deskripsi ebook 6. Edit konten di sini.</p>
        <button
          type="button"
